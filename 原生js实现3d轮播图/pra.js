...
function show() {
    ...
    for (var i = 0; i < mLen; i++) {
        ...
        oImg[lNum].style.transform = 'translateX(' + (-100 * (i + 1)) + 'px) translateZ(' + (60 - i * 100) + 'px) rotateY(40deg)';
        rNum = curDisplay + i + 1;
        if (rNum > len - 1) {
            rNum -= len;
        }
        oImg[rNum].style.transform = 'translateX(' + (100 * (i + 1)) + 'px) translateZ(' + (60 - i * 100) + 'px) rotateY(-40deg)';
    }
    // 当前显示图片直接z轴向前移动
